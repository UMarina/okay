                    ...
                    <form class="fn_variants" action="/{$lang_link}cart">
                        <div class="row">
                            <div class="col-sm-6">
                                {* Product variants *}
                                {if $product->radio}
                                <div class=" variant_radio {if $product->variants|count < 2} hidden{/if}">
                                    {foreach $product->variants as $v}
                                    <div>
                                     <input class="fn_variant form-check-input" type="radio" {if $v->id==$product->variant->id} checked {/if} id="variant{$v->id}" name="variant" value="{$v->id}" data-price="{$v->price|convert}" data-stock="{$v->stock}"{if $v->compare_price > 0} data-cprice="{$v->compare_price|convert}"{/if}{if $v->sku} data-sku="{$v->sku|escape}"{/if} {if $v->units}data-units="{$v->units}"{/if}>
